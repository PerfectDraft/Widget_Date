...
export default async function handler(req: any, res: any) {
  ...
  const timeStart = startTime || '18:00';
  const timeEnd = endTime || '22:00';
...
QUAN TRỌNG: Trường "time" của mỗi activity PHẢI nằm trong khung giờ từ ${timeStart} đến ${timeEnd}. Không được đặt giờ ngoài khoảng đó.`;

  const userMsg = `3 combo hẹn hò tại ${location || 'Hà Nội'}, sở thích: ${(preferences || []).join(', ') || 'Café, Ẩm thực'}, ngân sách: ${budget || '500000'}VND, thời tiết: ${weather || 'bình thường'}${companion ? `, đi cùng: ${companion}` : ''}. Thời gian hẹn hò: từ ${timeStart} đến ${timeEnd}. Mỗi activity phải có "time" nằm trong khoảng thời gian này. Mỗi combo 2-3 hoạt động.`;
